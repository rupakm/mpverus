...
// GLOBAL GRANT MONOTONICITY IS NOT REACHABLE HERE, and the reason is structural
// rather than a missing lemma. Grants travel on `acq_rsp(w)`, one channel per
// writer, so relating a grant to grants sent to other writers would need a gate
// that sees more than one channel. The server's counter is the state that would
// settle it, and it is a local variable nobody can read.
//
// Confirmed both ways on branch `refinement-experiment`: a refinement mapping
// over the message histories cannot express it (commit a69d422), and putting the
// counter in the machine can (`spike/pstate_grants.rs`). See docs/plan.md,
