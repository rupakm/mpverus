...
// SPIKE, not integrated. Can a service CONSUME `extra` -- a guarantee about
// pairs of messages -- on a channel it does not own?
//
// Today `extra` is write-only: it is proved as a machine invariant and there is
...
impl Pair<u64> for Hb {
    ...
    open spec fn extra(sent: Map<ChanId, Seq<u64>>) -> bool {
    ...
    open spec fn extra_gives2(c: ChanId, m1: u64, m2: u64) -> bool { m1 < m2 }

    proof fn lemma_extra_init(chans: Set<ChanId>) { }

    proof fn lemma_extra_preserved(sent: Map<ChanId, Seq<u64>>, c: ChanId, s: Seq<u64>, m: u64) {
